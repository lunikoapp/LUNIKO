...
function Demo() {
  const [messages, setMessages] = useState([{ from: 'luniko', text: 'Hello, curious human. What would you like to look at together?' }]);
  ...
  const saveLatestSpark = () => {
    ...
    if (lastQuestion && !sparks.includes(lastQuestion.text)) saveStorage(STORAGE.sparks, [...sparks, lastQuestion.text]);
    setSaved(true);
  ...
  const send = (prompt = input) => {
    ...
    if (!clean) return;
    setMessages((current) => [...current, { from: 'you', text: clean }, { from: 'luniko', text: responseMap[clean] || 'That is a wonderfully open question. Let’s slow it down: what part feels most interesting to you right now? We can make a list, draw a comparison, or try a tiny experiment.' }]);
    setInput('');
  };
  return <PageFrame><main className="mx-auto max-w-[1240px] px-5 py-12 md:px-8 md:py-20"><div className="grid gap-14 md:grid-cols-[.65fr_1.35fr]"><div className="reveal"><p className="mono text-[#777091]">Open studio / guided demo</p><h1 className="display mt-6 text-5xl leading-[.93] md:text-7xl">Follow a<br /><span className="text-[#777091]">spark.</span></h1><p className="mt-7 max-w-sm text-base leading-7 text-[#615b70]">This little conversation is available to everyone. Pick a door, then see where it leads.</p><div className="mt-10 grid gap-2">{Object.keys(responseMap).map((prompt) => <button key={prompt} onClick={() => send(prompt)} className="group flex items-center justify-between border-b border-[#c9c1c9] py-3 text-left text-sm hover:text-[#777091]" data-testid={`button-prompt-${prompt.slice(0, 8).replaceAll(' ', '-').toLowerCase()}`}><span>{prompt}</span><ArrowUpRight className="h-4 w-4 opacity-50 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" /></button>)}</div></div><div className="reveal reveal-2"><div className="overflow-hidden border border-[#615b70] bg-[#eee2d1] shadow-[10px_10px_0_#292641]"><div className="flex items-center justify-between border-b border-[#bcb2b5] bg-[#e6deee] px-5 py-4"><div className="flex items-center gap-3"><Mark small /><span className="mono text-[#777091]">safe mode / on</span></div><button onClick={() => setMessages([{ from: 'luniko', text: 'Hello, curious human. What would you like to look at together?' }])} aria-label="Reset conversation" className="text-[#615b70] hover:text-[#292641]" data-testid="button-reset-demo"><RotateCcw className="h-4 w-4" /></button></div><div className="min-h-[25rem] space-y-5 p-5 md:min-h-[29rem] md:p-8">{messages.map((message, index) => <div key={`${message.from}-${index}`} className={`flex ${message.from === 'you' ? 'justify-end' : 'justify-start'}`} data-testid={`message-${message.from}-${index}`}><div className={`max-w-[85%] border px-4 py-3 text-sm leading-6 ${message.from === 'you' ? 'border-[#292641] bg-[#292641] text-[#f8ebd2]' : 'border-[#bbb0bf] bg-[#f6ecd9]'}`}>{message.text}</div></div>)}{saved && <div className="flex items-center gap-2 text-xs text-[#687b28]" data-testid="status-saved"><Check className="h-3.5 w-3.5" /> Spark saved to your local space</div>}</div><form onSubmit={(event) => { event.preventDefault(); send(); }} className="flex gap-2 border-t border-[#bcb2b5] bg-[#e6deee] p-4"><label htmlFor="demo-input" className="sr-only">Ask LUNIKO</label><input id="demo-input" value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask a small, big question..." className="min-w-0 flex-1 bg-transparent px-2 text-sm outline-none placeholder:text-[#777091]" data-testid="input-demo-question" /><button type="button" onClick={saveLatestSpark} className="grid h-10 w-10 shrink-0 place-items-center border border-[#81798e] hover:bg-[#f6ecd9]" aria-label="Save spark" data-testid="button-save-spark"><Star className="h-4 w-4" /></button><button type="submit" className="grid h-10 w-10 shrink-0 place-items-center bg-[#c9f22d] hover:bg-[#d8fa54]" aria-label="Send question" data-testid="button-send-question"><Send className="h-4 w-4" /></button></form></div><p className="mt-5 flex items-center gap-2 text-xs text-[#777091]"><ShieldCheck className="h-4 w-4" /> This demo is curated for gentle, age-aware exploration.</p></div></div></main></PageFrame>;
}
